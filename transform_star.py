...
var_list = ['angle', 'factor', 'count', 'delta_x', 'delta_y', 'spokes']
option_map = {'angle':'-a', 'factor':'-f', 'count':'-n', 'delta_x':'-x', 'delta_y':'-y','spokes':'-z'}
option_vals = {'-a':'0.0','-f':'1.0','-n':'1','-x':'0.0','-y':'0.0','-z':0}
...
# check for user defined spoke number
if spokes != 0:
    angle = 2*3.14159/spokes
    count = int(spokes)
